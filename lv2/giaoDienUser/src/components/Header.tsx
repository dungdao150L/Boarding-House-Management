...
import { Bell, MessageSquare, Search, Languages, Check, X } from 'lucide-react';
...
export default function Header({ 
  ...
  return (
    <header className="fixed top-0 right-0 w-full lg:pl-64 flex justify-between items-center h-16 px-6 md:px-10 bg-[#ffffff] dark:bg-[#1c1d1f] border-b border-[#e7e8e9] dark:border-[#2e3132] z-30 transition-colors duration-200">
      {/* Title / Resident Portal */}
      <div className="font-bold text-lg text-brand-primary dark:text-[#ffffff] tracking-tight">
        {title}
      </div>
      ...
      <div className="flex items-center gap-4">
        {/* Optional Search */}
        {showSearch && (
          <div className="hidden md:flex items-center bg-[#f3f4f5] dark:bg-[#2e3132] rounded-full px-4 py-2 border border-[#c5c6cf] focus-within:border-brand-primary focus-within:ring-1 focus-within:ring-brand-primary transition-all">
        ...
        >
          ...
          />
          <span className="hidden sm:inline text-xs font-bold text-brand-primary">Nguyen Van A</span>
        </div>
      </div>
    </header>
  );
