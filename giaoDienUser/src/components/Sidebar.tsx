...
export default function Sidebar({ activeTab, setActiveTab, lang, onBookAmenityClick }: SidebarProps) {
  ...
  return (
    <aside id="sidebar-panel" className="hidden lg:flex flex-col h-screen w-64 fixed left-0 top-0 bg-[#ffffff] dark:bg-[#1c1d1f] border-r border-[#e7e8e9] dark:border-[#2e3132] py-6 px-4 z-40 transition-colors duration-200">
      {/* Brand Logo */}
      <div className="flex items-center gap-3 mb-10 px-2">
        ...
        <div>
          <h1 className="text-xl font-bold text-brand-primary dark:text-[#ffffff] tracking-tight">LuxLiving</h1>
          ...
          return (
            <button
              ...
              className={`flex items-center gap-3 px-4 py-3 rounded-lg text-left transition-all duration-150 active:scale-[0.98] ${
                isActive
                  ? 'border-l-4 border-brand-secondary-container bg-brand-surface-bg text-brand-primary font-bold dark:bg-brand-primary-container dark:text-[#ffffff]'
                  : 'text-on-surface-variant hover:text-brand-primary hover:bg-[#f3f4f5] dark:hover:bg-[#2e3132]'
              }`}
            >
              <Icon className={`w-5 h-5 ${isActive ? 'text-brand-primary dark:text-[#ffffff]' : 'text-on-surface-variant'}`} />
      ...
      <div className="mt-auto flex flex-col gap-4">
        ...
        <div className="border-t border-[#e7e8e9] dark:border-[#2e3132] pt-4 flex flex-col gap-1.5">
          <button 
            onClick={() => setActiveTab('settings')}
            className={`flex items-center gap-3 px-4 py-2.5 rounded-lg text-left text-sm font-medium transition-all duration-150 ${
              activeTab === 'settings'
                ? 'bg-brand-surface-bg text-brand-primary font-bold dark:bg-brand-primary-container dark:text-[#ffffff]'
                : 'text-on-surface-variant hover:text-brand-primary hover:bg-[#f3f4f5] dark:hover:bg-[#2e3132]'
            }`}
          >
